...
- **诚实空态(never fabricate)** —— ① 段内列表为空时走复用组件的「暂无公告」/「暂无相关新闻」, 本文件不往段内塞任何占位/示例条目; ② 段头条数**只在 > 0 时渲染**(首拉在途两数组皆为 `[]`, 恒显「共 0 条」会把"加载中"读成"确无内容"); ③ 但恢复组件的取数**把失败静默降级为空列表**(`loadNews`/`loadAnnouncements` 的 `catch { setX([]) }`, hook 未暴露这两个端点的失败位)⇒ 屏上「暂无公告」既可能是「该时间窗内确无内容」也可能是「取数失败」。故两段之上加**一行常驻说明** `data-testid="news-empty-caveat"`「列表为空时「该时间窗内确无内容」与「取数失败」在此不可区分」, **不声称**空列表就是"没有内容"; 根治(区分二者)需 provider 给这两个端点增失败位, 属跨任务 API 面(见 report concern 2)。
