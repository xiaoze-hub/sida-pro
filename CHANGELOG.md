...
- **xiaoze 协作流程定型**: cron `45d3592196a4` (Hermes→小主机 /root/sida-pro 源码同步, 每小时整点) — xiaoze 改本地代码前必须先打招呼, 我暂停 cron → 它改完发附件 → 我整合 commit + push → 恢复 cron → 部署 → 它自验. cron 暂停期间 Hermes 主机 commit + push **不会**覆盖小主机 `/root/sida-pro` 改动 (单向同步, 无反向). 路径澄清: xiaoze 工作目录 = 小主机 `/root/sida-pro`, **不是** `/mnt/c/Users/...`.
