...
- **落位(spec §三 去重表, 每数据点唯一归属)** —— ① 十档买卖额双向条 + 盘口形态/买盘占比/OB 失衡 + 快照时间; ② L2 成品资金(主力净额/主买净额/总买卖量/撤买卖量/逐笔成交·委托笔数); ③ 盘口演变事件(托单/压单/撤单/幽灵单明细)+ 幽灵单占比; ④ 主力意图(方向/净额/参与度/买占比/超大单·大单净额/尾盘净额/5 日阶段)+ 封单成色(成色/撤单率/撤单异动 z-score/封板成功率/撤单偏向/当前封板); ⑤ 资金流水(明盘·暗盘净额表, 最近 30 行); ⑥ 暗盘资金 TQ(超大/大/中/小单净额 · 拆单委托/平均拆单份数/撤单比/撤买卖量 · 托盘压盘锁盘); ⑦ 筹码(筹码峰/成本带/获利盘比例)。**不渲染**现价/涨停价/个股名/封单额(带1 拥有), 也不重复右栏「盘口速览」的五档口径。
