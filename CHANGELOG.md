...
"该日无数据行"→ 全部公式跳过 → `fetch_formula_signals` 返回 **0 行**，
而 `sync_formula_signals` 报"无任何公式返回数据(非交易日 / TQ 客户端未开 / 公式名不可用)"
—— 看起来像"今天没有信号"，**实际是参数没归一**（2026-09-25 验证时踩到，误判了一轮）。

新增 `_norm_day()`：接受 `2026-09-24` / `20260924` / `2026/09/24` / `2026.09.24`，
统一成紧凑格式后再发给客户端；`fetch_formula_signals` 与落库的 `trade_date` 一并归一。
测试补 1 项（ISO/紧凑/斜杠/空串/带空格）。

**为什么值得单独发版**：这是"静默空跑"类缺陷 —— 不报错、不写 0、只是没数据，
排查时极易误判成"非交易日"或"客户端没开"（本次就是这么绕了一轮的）。
...
在既有全市场条件选股扫描（`tq_formula_signals`，调度器/落库/API 都已存在）里加一组
**量价异动 + K线形态**：`C116 放量上攻`、`C117 温和放量上攻`、`C112 阶段放量`、
`C113 持续放量`、`C118 突然放量`、`C131 下跌多日再放量上涨`、`C120 小步碎阳`、
`C123 突破长期盘整`、`RED4 四串阳`、`C107 连续N天收阳线`、`C134 昨日底部十字星`、
`C135 价量渐低后阳包阴`、`MSTAR 早晨之星`、`SUNBY 阳包阴`。

**acCode 全部取自客户端 `formula_all(1)` 实测输出**（不手写猜，写错就静默 0 命中）。
新增一行即生效：落库、基线、API、调度全都不用改。

### feat-新增 chat 工具 `get_early_signals`（第 31 个 core 工具）

读 `tq_formula_signal_daily`，输出每个公式**今日触发家数 + 近 N 日基线 + 倍数**（按倍数降序），
并给出命中清单（前 12 只 + 总数）。问「今天有哪些票刚启动/刚放量」「有没有突破形态」时触发。

**纪律**：`complete=0`（扫描有分片失败）与 `truncated=1`（命中超 500 只）**无论有没有命中清单
都要显式警示**（这是本轮测试抓到的缺口：原先只在有清单时才提示）；基线缺失显示 `—` 不是 0；
文案明说"**不是推荐**"。

### feat-新增 `src/core/market_breadth_scheduler.py`（F2 收尾之一）

每交易日 **18:40** 从 PG klines 自算并落库广度序列（含启动回补：表内 <20 行时补一次）。
与 TQ 类调度器（15:35 情绪 / 15:50 条件选股）**错开** —— TQ 背后只有一个客户端进程。
已挂到 `src/bootstrap/startup.py` + `runtime.py`（与既有调度器同款写法）。

### research-自研广度口径 vs 客户端官方口径（交叉验证，同一天）

| 口径 | 涨/跌/平 | 总数 | 涨跌比 |
|---|---|---|---|
| 客户端 `tq.breadth`（全市场快照）| 1120 / 4306 / 151 | **5577** | 0.26 |
| 我方（PG klines 自算 09-24）| 458 / 1618 / 67 | 2143 | **0.28** |

**结论**：同一天涨跌比 0.28 vs 0.26 **高度一致**，差异来自**覆盖度**（PG 最新日只收全约 40%）
而非口径分歧；我方 09-23→09-24 比值 0.48→0.28 与情绪温度 42→39 的下行方向一致。
⇒ 自研口径可用，且 `symbols`/`complete` 字段如实反映覆盖度，消费者不会误读。

### test-新增 `tests/test_startup_signals.py`（6 项）

无数据如实 `ok=False`、按倍数排序与 ratio 计算、基线缺失为 `None` 而非 0、
truncated/incomplete **无清单也要警示**、文案含"不是推荐"、空 payload 返回空串。
相关套件共 **59 passed**。

> 未做（明确记录）：广度**情绪温度曲线前端页面**仍未做（数据/API 已就绪）。
...
### fix-`sentiment_score` 逐日全为 NULL

算法层只在 `latest_summary()`（最新一日）里算温度，逐日落库路径没有算 → `market_breadth_daily.sentiment_score`
**318 行全 NULL**。修：`compute_series_with_percentile()` 内**逐日**合成温度（落库要每天的值）。

### fix-`complete` 判定用固定阈值 1000 太松，会污染分位基线

现象：2026-09-24 库内只有 **2143 只**（klines 还在补 —— 30 分钟前才 559 只），却因为 2143 > 1000
被判为 `complete=True`，于是"半收状态的当日"进了分位基线。

修：改为**相对近期中位**判定 —— `阈值 = max(1000, 近期中位 × 0.8)`，自动适配市场标的数变化。
- 最新一日未收全时：`complete=False` + 渲染文本追加"⚠️ 该交易日标的数偏少（数据未收全）"，
  但其**原始数字照常报出**（该日确实发生了这些涨跌），且该日的指标/分位仍会计算
  （把它追加在完整序列末尾，不影响前面各日已算好的分位）。
- 新增返回字段 `median_symbols` / `complete_threshold`，让消费方能看到判定依据。

### test-新增 4 项（共 25 项，含算法层）

温度逐日落库非空（并明确"样本<20 的早期日该为 None"是对的行为）；完整度相对中位判定
（薄日必须 `complete=False` 且原始数字照常 + 渲染带警示）；整日完整时为 `True`。
连同 chat 注册表护栏共 **47 passed**。
...
108 个条件选股 / 15 个专家系统，产品只用了个位数），但**客户端公式必须先在该客户端注册
才能被 TQ 网关调用**（实测 21 个高价值公式全部 `ErrorId=9: no find formula setting`，
且 `formula_set_data*` 只能设数据、无法注册公式）。故"情绪周期量化"改为**自研实现**。

### feat-新增 `src/core/market_breadth.py`（纯函数、零 I/O）
...
| BTI 广量冲力 | `MA(上涨占比×100, 10)`；≥61.5 且前 10 日曾 ≤40 记 `bti_thrust` |
| MCL 麦克连 | `EMA(涨跌差,19) − EMA(涨跌差,39)`，另附累计 Summation |
| STIX 指数平滑广量 | `EMA(上涨占比×100, 6)` |

另出 `percentile_rank()`（历史分位，样本<20 不给）与 `sentiment_score()`（各指标分位合成
0-100 情绪温度），以及 `compute_series_with_percentile()` 供画温度曲线。

**口径纪律**：`latest_summary()` 输出带 `caliber` 字段，明写"自研口径（教科书定义）；
非通达信客户端公式，数值可能有差异"——避免与客户端同名指标混淆。分母缺失/为 0 一律
返回 `None`，**绝不用推测值填充**；空输入返回 `{'ok': False, 'reason': 'no_data'}`。

### test-11 项测试（含手算校核）

ADR 十天均比、ARMS=涨跌比/量比、MCL 首值为 0（两 EMA 同起点）、下量 0 时 ARMS 为 `None`、
缺失成交量不填 0、分位边界与样本不足、全缺失时温度为 `None`。`11 passed`。

> 流程说明：本提交因直接落在 `main` 且首版漏带 CHANGELOG（均违反 `AGENTS.md` 分支/提交规范），
> 本条为补救；该功能**剩余工作（全市场取数、落库迁移、API、AI 工具、前端）将走 `feat/` 分支**。

## 2026-09-25 (feat: TQ 第二批接入 —— 跨市场检索 / 指数ETF折溢价 / 客户端喂数据)

沿用第一批流程：**先真网关对拍契约 → 查重 → 接入 → 测试 → 发版 → 生产验证**。
三个 vendor 封装本就存在且结构正确（与 `kzz_info` 恒空那种真 bug 不同），只是无人调用。

| 新工具 | 数据源 | 回答什么 |
|---|---|---|
| `search_symbols` | `get_match_stkinfo` | **跨市场**证券检索：名称/简称/拼音/代码 → 带市场的代码 |
| `get_index_etfs` | `get_trackzs_etf_info` | 跟踪某指数的 ETF：现价 / IOPV / **折溢价率** / 规模 |
| `download_client_data` | `download_file` | 触发**客户端**下载数据文件（前置动作，非取数）|

实测要点（都已写进 caliber 与文档，避免后人再踩）：

- **检索跨市场**：`宁德时代`→`300750.SZ` **+ `03750.HK`**；`002361`→股票 **+ `002361.OF`**（场外基金）；
  `立讯转债`→`128136.SZ`。**查不到返回 `None` 而不是 `[]`**。同名跨市场，结果标出市场
  （否则港股会被当 A 股用）。仓库原有 `search_stocks()` 只覆盖 A 股，故此工具确有增量。
- **`trackzs_etf`**：`950162.CSI` / `000300.SH` / `399006.SZ` 可用；无 ETF 的指数（如 `930599.CSI`）
  返回 `{'raw': ''}` 空壳（已归一为空列表）。工具额外算折溢价率 `(现价-IOPV)/IOPV`，
  **IOPV 缺失或为 0 时不给数字**（不编），结果按规模降序。
- **`download_file` 的 `down_time` 必须是 `'YYYYMMDD'` 字符串**：传 int → `ErrorId=10 RPC处理异常`。
  新增 `_norm_down_time()` 统一归一 int / datetime / date / `'2026-09-24'`；归不了一律原样交客户端报错，
  **不替它猜日期**。返回只有客户端回执（`{'ErrorId':'0','Msg':'下载经营分析数据文件[2026]成功。'}`），
  文件落客户端 `.\\PYPlugins\\data` —— 工具文案明说"**不是取数结果**"，防止被当查询用。
- 裸码后缀自愈沿用 `_quiet=True`，中间失败不留假 WARNING。

同步：`tests/test_w33_chat_registry.py` 清单护栏 44 → **47**（`EXPECTED_TQ` 六件套）；
`docs/tq-interface-coverage.md` 三行转「已接」（并注明**原 62 的分解口径未重算**，别相加）；
新增 8 个测试（日期归一 / 跨市场标注 / None 空结果 / 规模降序+折溢价 / 缺参不猜 / 客户端回执转述），
相关测试 45 passed。
...
`datetime.now().weekday() < 5` —— 而全仓的交易日判定早在 W2.6/B6 就**收口到
`src/core/trading_calendar.py`**, 该模块 docstring 明写"不要再走 weekday()<5 之类的旧判定"。

- 危害: 中秋(2026-09-25 周五)/国庆这类**周中假日**会被当成交易日 → 休市日空跑一轮
  全市场扫描(26s, 打客户端), 回补还会往里喂不存在的日期。
- 修法: `_is_market_day()` 走 `is_trading_day()`; `_recent_trading_days(n)` 改为按
  `prev_trading_day()` 逐个倒推**真实交易日**; 日历静态表未覆盖的年份按**非交易日**
  处理并告警(宁可漏采, 不在假期乱动数据)。
- 顺带: `tq_sentiment_scheduler` 缺 `zoneinfo` 导入(新代码会 NameError), 已补;
  条件选股调度器删掉不再使用的 `timedelta`。
- 回归测试: 断言 `_recent_trading_days(6)` 返回的每一天都必须是日历认可的交易日 ——
  这条在旧实现下、逢假日/周末运行必挂。

## 2026-09-25 (feat: 通达信三个「有接口没人用」的能力盘活 —— 打新日历 / 转债条款 / 板块归属)

啃完 TQ 官方接口文档(231 页, 59 个接口)后做的一波: 挑出**只有通达信给、产品却完全没接**
的三个能力。它们在 vendor 里其实早已实现, 只是**没人调用**, 而且其中两个是坏的:

- `ipo_info` 老版只透传裸 list(字段全字符串、无类型说明) → 现结构化, 带 `ipo_type_cn`;
- `kzz_info` 判 `isinstance(v, dict)`, 而真实响应是 `result.Value = [ {...} ]` →
  **恒返 `{}`**: 用户问"这转债强赎价多少"永远得到"查不到"; 现取首行, 并标定
  **必须传带后缀代码**(裸码报 codestr error);
- `relation`(板块归属)本来就是对的, 单纯没人用。

新增三个对话工具(数据源 = 通达信客户端, 零外部配额、零费用):

| 工具 | 回答什么 | 说明 |
|---|---|---|
| `get_ipo_calendar` | 今天/最近有什么新股、新债可打新 | 申购日/价/代码/上限/发行市盈率; `kind=new/bond/all` |
| `get_kzz_terms` | 某转债的条款(转股价/强赎/回售/到期/纯债价值/评级) | 裸码**自动补 .SZ/.SH 后缀**再试, 不猜死 |
| `get_stock_sectors` | 这只票属于哪些板块 | 行业/概念/地区/风格/指数分组, 用于题材归因 |

**诚实口径**(这一版的重点): 客户端用 `0` 表示"字段还没出来" —— 工具一律渲染成
**待定/未披露**, 绝不写成"申购价 0 元""0 倍市盈率"; TQ 不可用时如实说取不到,
**不用其它源猜一个数字顶上**(与全站幻觉口径一致)。

验证: 真网关实测 20260924(新股 1 条 / 今天及以后 4 条 / 新债 4 条)、立讯转债 128136.SZ 条款、
002361.SZ 板块归属; 新增 11 个单测(含 `kzz_info` 恒空回归、裸码自愈、0 值渲染、降级文案)全过。

## 2026-09-25 (fix: 条件选股非交易日不再落 0 —— 0 会把基线拉低)

接上一版: 条件选股信号日序列落库时,**非交易日会写 10 行 hit_count=0**。

- 危害: 基线是"近 N 日均值", 假期那几行 0 会把基线系统性拉低 ——
  "今天 38 家 vs 均值 20 家"里那个 20 是被人造 0 稀释过的。下一个会踩到的
  是 10-01~10-07(周中假期, 日常任务照跑 Mon-Fri)。
- 修法: 两种 0 在响应里**本就可分** —— 该日期**一行数据都没有**(`date_rows=0`)
  = 非交易日/窗口未覆盖; 有数据行但值全 0 才是真的"0 家触发"。
  新增 `tq._date_rows_seen()`, `formula_scan` 返回 `date_rows`/`date_has_data`,
  采集器对 `date_has_data=False` 的日期**跳过不落库**(记日志)。
- `sync_formula_signals` 的报错文案改准: 原来是"TQ 无返回(客户端未开)",
  现在明确列出含"非交易日"在内的四种可能。

**验证**: 36 个测试全过(新增 `test_scan_flags_no_data_day` /
`test_fetch_skips_no_data_day_instead_of_writing_zero`, 断言非交易日数据库里 0 行)。

## 2026-09-25 (feat: 通达信条件选股信号 —— 108 个公式的全市场「当日触发家数」)

通达信客户端 TQ 网关的 `formula_process_mul_xg`(条件选股)接进来, 给市场宽度补一个
**免费源没有**的维度: 每个技术条件今天有多少只票触发。

**为什么值得做**
- 数据源 = 本地 TQ 客户端, **无限频、零外部依赖、零成本**(对照: 问财限频 250ms)。
- 东财/同花顺免费层只有"当日快照", 没有"某条件当日触发家数"的口径, 更无历史 → 算不出基线。
- 落库后才能给**相对基线**: "今天 38 家 MACD 买入 vs 近 20 日均值 12 家" = 技术面集体转强。

**新增**
- `marketdata.vendors.tq.formula_scan()` — 全市场扫描(500 只/批 × 12 批, 实测 **24.2s** 扫完 5577 只);
- `formula_xg_mul()` **修分页 bug**: 服务端按 `batch_stock_count=800`/页分页并给
  `next_stock_index`/`has_more_batch`, 老实现只取首页 → 大列表**静默漏标**(数字看着正常却是残缺的);
- `src/collectors/tq_formula_signals.py` + 迁移 `_m179_tq_formula_signal_daily` +
  每交易日 15:50 调度 `TqFormulaSignalScheduler`(与情绪序列 15:35 **刻意错开** ——
  TQ 背后只有一个 Windows 客户端进程, 同点跑会把它压出假死);
- API `GET /api/formula-signals`(各公式家数 + 近 20 日基线) 与
  `GET /api/formula-signals/{code}`(日序列 + 最近一日命中清单)。

**诚实性(硬约束)**
- `complete=False` = 该日有分片失败 ⇒ `hit_count` **不是**全市场家数; 标记一路带到响应, 消费方必须标注。
- 取不到代码池 / 全部公式失败 → 显式 `error`, 单公式失败只少一行(下次补), **不写 0** ——
  0 会被读成"今天没人触发"(编造)。
- 无采集数据 → 接口返回 `degraded: true` + 说明, 不当"今日无信号"。

**实测记录(踩坑留证)**
- 500 只/次 = 1.6s; 一次传全市场则由服务端按 800/页分页 = 7 页 33s; 50 只/次要 112 次 ≈ 145s(踩过)。
- 不带时间窗(`count=-1`=全部历史)会把全市场扫描从 24s 拖到分钟级 → 扫描强制窄窗口(T-20 自然日)。
- 信号键 = `OUTPUT1`(不是公式名), 命中 = `Value "1"`; fixture
  `tests/fixtures/tq_formula_mul_xg_sample.json` 是从真网关抓的**未改写**原始响应。

**验证**: 新增 34 个测试(翻页合并/ErrorId=19 部分返回/游标不前进不死循环/分片失败标记/
单公式失败不写 0/落库幂等/基线无历史给 None/端点 404 + 降级);
门禁全过(迁移校验 v101-v179、作用域查询、is_pg、lock 覆盖); 覆盖率 57.32% > 基线 50%。
...
## 2026-09-24 (fix: 股东户数/分红生产恒为空 —— Symbol 后缀未归一化 + 接 TQ 备源)

发现并修掉一个**跨能力**的静默故障: 生产上 `股东户数`/`分红` 长期返回**空数组**。
根因不是数据源不可用, 而是取数路径里的 4 处命名/口径错:

1. **`Symbol.parse` 不剥交易所后缀**。`Symbol.parse("002361.SZ","CN").code` 得到
   `"002361.SZ"`, 而各 vendor 的代码字段一律要**裸码**: 东财 filter
   `SECURITY_CODE="002361.SZ"` 恒 0 条(裸码 3 条), TQ 的 `to_tq_code` 因 `len!=6`
   直接返回 None。端点把 URL 原文直接喂给 `md.shareholders()`/`md.dividend()`,
   于是两能力全灭; 叠加智兔股东接口 429 → 生产恒空。现按已知交易所后缀
   (SH/SZ/BJ/HK)归一化到 `code` + `market`; 刻意**不贪心剥 `.`**, 否则 `BRK.B`
   会被毁掉(无后缀/美股/HK 行为不变)。
2. **东财分红 `PRETAX_BONUS_RMB` 是"每10股"口径, 却被当"每股"用** → 差 10 倍。
   标定证据: 该字段 0.5 对应方案原文"10派0.50元"; 前端 `DataSources.tsx` 直接按
   "元/股"显示。同尺度参照: 智兔 vendor 一直用 `send/10`。现统一 `/10`。
3. **东财分红 `transfer_ratio` 取的是本报表里不存在的键 `TRANSFER_RATIO`** → 转增恒为
   None。真实字段是 `IT_RATIO`(转增)/`BONUS_RATIO`(送股); 实测 2013-04-16
   `IT_RATIO=8 / BONUS_RATIO=2` ↔ 方案"10送2.00转8.00"。
...
实测标定(真 TQ 网关 + 真东财, 非 mock):

| 项 | 002361.SZ | 600519.SH |
|---|---|---|
| 股东户数 TQ vs 东财 | 264938 = 264938 | 296404 = 296404 |
| 分红派息对拍(容差内) | 13/13 笔 | 20/20 笔 |
| 送转合计 TQ vs 东财送+转 | 10.0 = 0+10 / 10.0 = 2+8 | 1.0 = 0+1 |

已知口径差(已写进 vendor docstring): TQ 把**送股与转增合并**(不分送/转)、无方案进度
...
## 2026-09-24 (docs: TQ 切换面审计 — 已接/未接/无等价 + 硬约束)

对"能用通达信客户端的接口全部换成 TQ"这条线做一次成体系盘点, 落 `docs/TQ切换面审计_20260924.md`
(带基线三要素: commit 1ac251a / VERSION v0.13.9; 并写明**没审**哪些)。

结论: TQ **封装层已完整**(`tq.py` 30+ wrapper), 但业务层只接了 9 处、registry 只注册 3 个能力
(`quote`/`kline`/`more_info`); 剩余工作不是"拿不到数据", 而是**按能力补接线 + 注册 + seed 行**。
未接且有等价的高价值项: 智兔(付费)承担的 `shareholders`/`dividend`(今日实测智兔 429)、
`board_capital_flow`/`market_capital_flow`(同花顺单点)、`margin`、涨跌停快照(炸板次数为 TQ 独有)。
无等价必须保持外部源的: 公告/新闻/互动易/F10 语义字段/海外/竞价逐笔。

同时写明切之前的 5 条硬约束: 口径红线(资金类必须带 caliber, 主力意图禁 get_capital_flow)、
单位(万元, 错 10000 倍不报错)、单次 ≤50 只(超了打挂客户端)、L2 字段盘后全 0、降级响应不得入缓存。
...
## 2026-09-24 (chore: 勘误幽灵 tag 镜像说明 + 忽略 .venv 软链)

- **勘误**: CHANGELOG 原称 v0.13.7"重指 tag 后为可部署版本", 与事实不符(ACR 无该镜像, 详见"镜像说明"节)。
  判据取自 `docker manifest inspect`/`gh run list` 实测, **不取自"我重跑过 CI"这个动作**。
- **v0.13.4 补标**: 构建失败且无 Release, 同为不可部署的幽灵 tag, 此前未标注。
- **`.gitignore`**: `.venv/` 匹配不到**软链**形式的 `.venv`(本机 `.venv -> /home/ubuntu/sida-venv`),
  改为 `.venv`; 否则 `git add .` 会把软链提交进仓库。
...
### 镜像说明: v0.13.4 / v0.13.6 / v0.13.7 均不可部署(ACR 无镜像)
- **v0.13.4**: ACR 构建 `completed/failure`(`35843887639`), **从未产出镜像**, 且**没有 Release 记录**
  —— 属**未标注的幽灵 tag**。其内容(市场级情绪日序列)已在后续版本里。
- **v0.13.6**: CI 在**产出镜像前**被取消(构建期间代码又被改进, 见上"早期返回"两条),
  它**没有可用镜像, 不要用这个 tag 部署**。tag 与 Release 保留仅为留痕。
- **v0.13.7**: ⚠️ **勘误 (2026-09-24)** —— 此处原写"重指 tag 后这是本轮的**可部署版本**", **是错的**。
  实测: 该 tag 的三次 ACR 构建(两次 `cancelled` + 一次 `completed/failure` `35891335563`)
  **从未产出镜像**; `docker manifest inspect <ACR>:v0.13.7` ⇒ 不存在; 小主机 `docker images` 亦无。
  v0.13.7 的 Release 正文("不可部署, 请用 v0.13.8")是对的 —— 与本条矛盾的是 CHANGELOG 自己。
  **判据只能取自 ACR 清单, 不能凭"我重指了 tag 并重跑了 CI"推断镜像已产出**(构建可能再次失败)。
  → 该版本的修复(网关故障分支的 TQ 涨跌家数 + 采样器早返回)全部包含在 **v0.13.8**。
