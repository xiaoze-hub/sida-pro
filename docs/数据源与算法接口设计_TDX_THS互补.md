...
| 明盘资金 | 单笔≥30万大单净额 | 盘中：thsdk `big_order_flow`按30万过滤；盘后：`.tck`逐笔聚合 | TQ `Zjl/Zjl_HB`成品（实时快照） | 三源阈值一致，可互验 |
| 暗盘资金 | AI识别拆单/对倒/量化单，代表真意图 | 盘后：`.tck`委托号聚簇（主动侧100%，`dark_flow_fusion`） | 盘中：thsdk逐笔拆单特征近似（金额分布/同价堆积） | 被动maker侧23.2%缺口，两源都看不到，必须标注confidence |
...
3. 每个资金数字必带：`source`（tck/thsdk/tq/kline_est）+ `confidence`（exact/approx/none）+ 周期（1/3/5日）。
4. 凭据：THS账号走`.env`（mx_8lj4le6qd），禁提交仓库；TQ网关仅小主机本地。
