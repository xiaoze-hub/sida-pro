...
设计:
- 静态表覆盖 2025-2028, 含 A 股交易所发布的法定节假日(春节/国庆/中秋等) + 调休补班
- 用集合 + O(1) 查询, 无外部依赖
...
- **2028 为预估表**(KI-028, 2026-09-18), 须在 2027-12-31 前按正式公告复核替换
"""
...
# KI-028 (2026-09-18): 2028 静态表 —— 国办 2028 节假日公告通常年底才出,
# 此处按农历/公历**预估**填充, 与 2027 同策略; 2027-12 前须按正式公告复核替换。
_HOLIDAYS_2028: set[str] = {
    # 元旦(2028-01-01 周六 → 01-03 周一补休)
    "2028-01-01", "2028-01-02", "2028-01-03",
    # 春节(预估 2028-01-26 除夕 ~ 02-01 初六; 实际以国办公告为准)
    "2028-01-26", "2028-01-27", "2028-01-28", "2028-01-29",
    "2028-01-30", "2028-01-31", "2028-02-01",
    # 清明(2028-04-04 周二)
    ...
    # 中秋(预估 2028-09-12 前后)
    "2028-09-12", "2028-09-13", "2028-09-14",
    # 国庆
    "2028-10-01", "2028-10-02", "2028-10-03", "2028-10-04",
    "2028-10-05", "2028-10-06", "2028-10-07",
}

_WORKDAYS_2028: set[str] = {
    "2028-01-22",  # 周六补班(春节前, 预估)
    "2028-02-05",  # 周六补班(春节后, 预估)
    "2028-04-29",  # 周六补班(劳动节前, 预估)
    "2028-05-06",  # 周六补班(劳动节后, 预估)
    "2028-09-30",  # 周六补班(国庆前, 预估)
}
